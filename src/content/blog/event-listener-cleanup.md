...
description: 'removeEventListener 를 불렀는데 리스너가 남아 있다면, 원인은 거의 항상 하나다. 등록할 때와 다른 함수 객체를 넘긴 것이다.'
pubDate: 'Sep 04 2026'
---

`removeEventListener` 를 분명히 불렀는데 리스너가 그대로 남아 있다면, 원인은 거의 항상 하나다. 등록할 때와 **다른 함수 객체**를 넘긴 것이다.

## 해제는 세 가지가 모두 맞아야 일어난다

`removeEventListener` 는 **이벤트 타입 + 리스너 객체(참조 동일성 `===`) + capture 플래그** 세 가지가 모두 일치하는 항목을 찾아 지운다. 함수의 *이름*이나 *내용*이 같은 것은 아무 의미가 없다.

등록할 때 넘긴 것은 "코드"가 아니라 그 순간 만들어진 함수 객체다. 지울 때는 그 객체를 다시 가져와야 한다.

## 인라인 화살표는 매번 새 객체를 만든다
...
화살표 함수라서 되는 게 아니다. **한 번 만들어 어딘가에 저장해뒀기 때문에** 되는 것이다. 클래스 필드 화살표는 그 저장을 언어가 대신 해주고, 덤으로 `this` 바인딩까지 해결해준다.

## 참조를 고정하는 네 가지 방법

### 1. 클래스 필드 화살표 — 기본값으로 쓰면 된다

인스턴스 생성 시 한 번 만들어져 필드에 담긴다. 등록·해제 순서나 호출 횟수를 따질 필요가 없다. 대가는 인스턴스마다 함수 객체 하나(프로토타입 공유 아님)인데, 리스너 몇 개 수준에서는 신경 쓸 비용이 아니다.
...
즉 `addEventListener(t, fn, {passive:true})` 로 등록했어도 `removeEventListener(t, fn)` 으로 지워진다. 반대로 capture 만 어긋나면 안 지워진다.

## 같은 함정이 반복되는 API들

"등록할 때 준 것을 그대로 돌려줘야 한다"는 규칙은 DOM 이벤트만의 것이 아니다.
...
> React 에서 인라인 화살표가 괜찮은 이유도 같다. 등록과 정리가 같은 클로저 안에서 같은 변수를 참조한다. 화살표 자체가 문제였던 적은 없다.
...
리스너를 다 뗐어도, 정리 전에 이미 걸어둔 비동기 콜백(`onReady`, `then`, 응답 핸들러)은 나중에 도착한다. 그 안에서 이미 파괴된 객체를 만지면 같은 종류의 null 참조 에러가 난다.
...
화면을 떠난 뒤에 나는 에러, 특히 `Cannot read properties of null` 계열이면 정리 실패를 먼저 의심한다. 결정적인 단서는 **같은 밀리초에 같은 에러가 여러 건** 찍히는 것이다. 진입·이탈을 반복할 때마다 죽은 리스너가 쌓였고, 이벤트 한 번에 그것들이 전부 발화했다는 뜻이다.
...
그 밖에 Performance monitor 의 JS event listeners 카운터가 우상향하는지, Memory 탭 힙 스냅샷에 detached 노드가 쌓이는지도 같은 신호다.

### 재현 조건을 먼저 확보한다

정리 실패는 이벤트가 실제로 발화해야 드러난다. 리스너 안에 조건 분기가 있으면 그 조건까지 만족시켜야 한다. 예를 들어 "모바일 폭 경계를 넘을 때만" 동작하는 핸들러라면, 창을 아무렇게나 줄이는 것으로는 재현되지 않고 경계를 가로질러야 한다. 고치기 전에 재현을 못 잡으면, 고친 뒤의 "에러 없음"은 아무것도 증명하지 못한다.
